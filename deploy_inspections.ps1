# ============================================================================
# TM Verified Inspection — deployment script
# Run from C:\tm-next in PowerShell with:
#   powershell -ExecutionPolicy Bypass -File deploy_inspections.ps1
# Writes 4 files as UTF-8 WITHOUT a BOM. Safe to re-run.
#
# IMPORTANT: after this runs, run supabase/migrations/003_inspections.sql
# in Supabase SQL Editor BEFORE deploying, or /admin/inspections will error.
# Also create a Storage bucket named "inspection-reports" (Storage > New
# bucket) if it doesn't already exist — same as "payment-receipts".
#
# NOTE ON VISIBLE CHANGE: search.ts is modified in this batch. The
# "Verified" badge on /browse will change from always-on (fake) to
# reflecting real inspection status — every listing will show as
# unverified until it goes through Verified Inspection. This is
# intentional (see 003_inspections.sql header comment for details).
# ============================================================================
...
Write-Host ""
Write-Host "Inspection feature files written. Run: git status" -ForegroundColor Green
