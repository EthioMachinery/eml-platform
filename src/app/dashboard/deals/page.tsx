...
export default function DealsDashboardPage() {
  ...
  async function loadEscrowTransactions() {
    setLoading(true);
    try {
      // Query the actual escrows table matching either buyer or seller ID
      ...
      console.error("Failed to query escrows table:", err);
    } finally {
      setLoading(false);
    }
  }
  ...
  const tabs = [
    { key: "all", label: t("All Deals", "ሁሉም ግብይቶች") },
    { key: "awaiting_funding", label: t("Pending", "በመጠባበቅ ላይ") },
    { key: "funded", label: t("Funded", "ክፍያ የገባ") },
    { key: "completed_payout", label: t("Completed", "ተጠናቋል") }
  ];
    ...
    <main className="min-h-screen bg-zinc-950 text-white pb-32">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            <div>
              ...
              <h1 className="text-5xl font-black leading-tight">
                {t("Enterprise Deals Command Center", "የድርጅት ግብይት መቆጣጠሪያ ማዕከል")}
              </h1>
      ...
      <section className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          <KpiCard title={t("Total Deals", "ጠቅላላ ግብይቶች")} value={escrows.length} icon={Briefcase} />
          <KpiCard title={t("Gross Revenue", "ጠቅላላ ገቢ")} value={`ETB ${formatter.format(totalRevenue)}`} icon={Banknote} />
          <KpiCard title={t("EML Commission", "የEML ኮሚሽን")} value={`ETB ${formatter.format(totalCommission)}`} icon={ShieldCheck} />
          <KpiCard title={t("Seller Net", "የሻጭ የተጣራ ገቢ")} value={`ETB ${formatter.format(totalNet)}`} icon={CheckCircle2} />
        </div>
      </section>
        ...
        {loading ? (
          ...
          <div className="space-y-7">
            {filteredDeals.map((deal) => {
              const machineryTitle = deal.listing ? `${deal.listing.brand} ${deal.listing.model}` : "EML Equipment Transaction";
              return (
                  ...
                  <div className="p-7 border-b border-zinc-800">
                    <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-5 text-zinc-400">
                          <div className="flex items-center gap-2">
                            <Banknote size={18} />
                            ETB {formatter.format(deal.total_amount)}
                          </div>
                          <div className="flex items-center gap-2">
                            <ShieldCheck size={18} />
                            Commission: ETB {formatter.format(deal.eml_commission_fee)}
                          </div>
                          <div className="flex items-center gap-2">
                            <Clock3 size={18} />
                      ...
                      <div className="flex flex-wrap gap-3">
                        <Link
                          href={`/payment?deal=${deal.id}&amount=${deal.total_amount}`}
