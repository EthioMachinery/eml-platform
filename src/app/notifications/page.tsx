...
export default function NotificationsPage() {
  ...
  async function loadNotifications() {
    ...
    try {
      ...
      const fallbackData: NotificationItem[] = [
        {
          ...
          title_am: "አዲስ AI ማገናኛ ተገኝቷል",
          message: "A verified excavator supplier matches your latest request.",
          message_am: "የተረጋገጠ ኤክስካቫተር አቅራቢ ከጥያቄዎ ጋር ተዛማጅ ሆኗል።",
          type: "match",
          created_at: new Date().toISOString(),
          read: false,
          action_url: "/smart-match",
        },
        {
          id: "2",
          title: "Transport Available",
          title_am: "መጓጓዣ ተገኝቷል",
          message: "Lowbed truck available for Addis Ababa → Bahir Dar route.",
          message_am: "ከአዲስ አበባ ወደ ባህር ዳር የሚሄድ ሎቤድ መኪና ተገኝቷል።",
          type: "transport",
          created_at: new Date().toISOString(),
          read: false,
        ...
        {
          id: "3",
          title: "Financing Pre-Approved",
          title_am: "ፋይናንስ ቅድመ ማጽደቅ",
          message: "Your machinery financing request has been pre-approved.",
          message_am: "የማሽነሪ ፋይናንስ ጥያቄዎ በቅድሚያ ጸድቋል።",
          type: "finance",
          created_at: new Date().toISOString(),
          read: true,
        ...
        {
          id: "4",
          title: "Insurance Offer Ready",
          title_am: "የመድን ቅናሽ ዝግጁ ነው",
          message: "AI generated machinery insurance quotation available.",
          message_am: "AI የፈጠረው የማሽነሪ መድን ዋጋ ተዘጋጅቷል።",
          type: "insurance",
          created_at: new Date().toISOString(),
          read: true,
      ...
      // Map columns dynamically: Convert 'body' to 'message' and 'link' to 'action_url' [1]
      const finalData = data && data.length > 0 
        ? data.map((item: any) => ({
            id: item.id,
            title: item.title,
            title_am: item.title_am,
            message: item.body || item.message || "No Details",
            message_am: item.body_am || item.message_am || "ዝርዝር መግለጫ የለም",
      ...
      setStats({
        ...
        unread: finalData.filter((n: any) => !n.read).length,
        aiAlerts: finalData.filter((n: any) => n.type === "match").length,
        finance: finalData.filter((n: any) => n.type === "finance").length,
        logistics: finalData.filter((n: any) => n.type === "transport").length,
      });
    } catch (err) {
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-3 mb-3">
                            <h3 className="text-2xl font-black">
                              {t(
                                notification.title,
                                notification.title_am || notification.title
                              )}
                            </h3>
                          ...
                          <p className="text-zinc-400 leading-8 max-w-3xl">
                            {t(
                              ...
                              notification.message_am || notification.message
                            )}
                          </p>
