...
export default function TMUniversalMarketplace() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-8" id="eml-marketplace-app">
      ...
      {/* PROMINENT BUY / RENT INTENT SELECTOR — the first, most obvious choice
          a visitor makes. Previously this was a small 3-button toggle buried
          in the sidebar filters, easy to miss, with only a tiny corner badge
          on each card distinguishing sale from rental listings. */}
      <div className="grid grid-cols-3 gap-3 mt-6">
        {([
          { key: "all" as const, label: "All Machinery", icon: "🏗️", activeClass: "bg-zinc-700 border-zinc-500" },
          { key: "sale" as const, label: "For Sale", icon: "🟢", activeClass: "bg-green-600 border-green-400" },
          { key: "rent" as const, label: "For Rent", icon: "🔵", activeClass: "bg-blue-600 border-blue-400" },
        ]).map((tab) => (
          <button
            key={tab.key}
            type="button"
            onClick={() => setRentFilter(tab.key)}
            className={`py-4 rounded-xl border-2 font-black text-sm md:text-base uppercase tracking-wide transition-all flex items-center justify-center gap-2 ${
              rentFilter === tab.key
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mt-8">
        <aside className="bg-zinc-950 border border-zinc-900 rounded-xl p-5 space-y-5 h-fit">
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg px-3 py-2 text-xs text-blue-300">
            Use the <strong>For Sale / For Rent</strong> buttons above to narrow by what you&apos;re looking for.
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {displayedListings.map((item) => {
                ...
                return (
                  ...
                  >
                    {/* Full-width sale/rental banner — replaces the old tiny
                        corner tag, which was too easy to miss when browsing
                        the mixed "All" view. */}
                    <div className={`py-1.5 text-center text-[11px] font-black uppercase tracking-widest text-white ${
                      item.isRentalOnly ? "bg-blue-600" : "bg-green-600"
                    }`}>
                    ...
                    <div className="relative h-48 bg-zinc-900 flex items-center justify-center border-b border-zinc-900 overflow-hidden">
                      {item.imageUrl && !failedImageIds.has(item.id) ? (
                        <img
                          ...
                          onError={() => {
                            // FIXED: this used to just hide the broken <img>
                            // (display: none) and leave an empty/garbled box
                            // behind, since React had already committed to
                            // the <img> branch of this ternary. Tracking the
                            // failure in state instead makes it actually
                            // re-render the clean text fallback below.
